# Python 代码规范
# 参考: PEP 8 + Google Python Style Guide

## 格式与风格

### 缩进与行长
- 使用 4 个空格缩进（非 Tab）
- 最大行长: 88 字符（Black 默认）或 100 字符
...
| 函数/方法 | snake_case | `get_user()`, `process_order()` |
| 变量 | snake_case | `user_name`, `order_count` |
| 常量 | UPPER_SNAKE_CASE | `MAX_RETRY_COUNT` |
| 私有 | _leading_underscore | `_internal_method()` |
| 强私有 | __double_underscore | `__very_private` |
...
# 使用
user = UserFactory()  # 默认数据
user = UserFactory(name="Custom Name")  # 覆盖字段
```
